...
export const registrationBreederVerificationSchema = z
  .object({
  ...
  .superRefine((data, ctx) => {
    if (data.business_type === "company") {
      ...
      if (!(data.liability_insurance instanceof File)) {
        ctx.addIssue({
          code: "custom",
          message: "Liability Insurance Image is required",
          path: ["liability_insurance_image"],
        });
      }
    }
...
export type RegistrationBreederVerificationFormValues = z.infer<
  typeof registrationBreederVerificationSchema
>;
