...
import { updateRegistrationServiceVerification } from "@store/actions";
...
import {
  ...
  RegistrationServiceVerificationFormValues,
  registrationServiceVerificationSchema,
...
const RegistrationServiceVerificationForm = () => {
  ...
  const {
    ...
    actions: {
      updateRegistrationServiceVerification,
    },
  });

  const methods = useForm<RegistrationServiceVerificationFormValues>({
    resolver: zodResolver(registrationServiceVerificationSchema),
    defaultValues: state.serviceVerification,
  });
  ...
  const handleSuccess = useCallback(
    (variables: RegistrationBreederVerificationFormValues) => {
      actions.updateRegistrationServiceVerification(variables);
    },
    [actions],
  );
  ...
  return (
    <TemplateAuthContent className="max-tablet:pb-12.25">
      ...
      <TemplateAuthContentFooter>
        <Box className="gap-4">
          <SubmitButton form="verification-form" formState={methods.formState}>
            Finish
          </SubmitButton>
...
export default RegistrationServiceVerificationForm;
