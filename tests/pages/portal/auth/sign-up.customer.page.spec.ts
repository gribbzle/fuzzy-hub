...
test.describe("visual regression tests", () => {
  ...
  test("sign up (customer flow)", async ({ page, viewport, msw }) => {
    msw.use(
    ...
    await test.step("finish", async () => {
      await page.locator("input[name=full_name]").fill("John Smith");
      await page.click("button[type=submit]");
      await expect(page.getByText("You are done!")).toBeVisible();
